...
from .range_linear import _quant_param_to_str,_enum_to_str, ModuleQuantMode, ClipMode, \
    _verify_enum_value, verify_quant_mode, verify_clip_mode, _get_saturation_fn, \
    _get_quant_params_from_tensor, _get_quant_params_from_stats_dict, _get_clipping_values,\
    TensorQuantMetadata, QuantSettings, linear_quantize_clamp_with_metadata, linear_dequantize_with_metadata, \
    UnsatisfiedRequirements, _check_clipping_val, FPWrapper, FP16Wrapper
...
# Post Training
###############################################################################
class RangeLinearQuantWrapperBS(nn.Module):
    """
    ...
    def __init__(self, wrapped_module, num_bits_acts, num_bits_accum=32, mode=LinearQuantMode.SYMMETRIC,
        ...
        super(RangeLinearQuantWrapperBS, self).__init__()
    ...
    def forward(self, *inputs):
        ...
        # # --- dump output activation non quantized version ---
        # out_orig = self.wrapped_module(*inputs)
        # if self.clip_half_range:
        #     out_orig = f.relu(out_orig)
        #
        # save_dir = getattr(msglogger, 'logdir', '.')
        # save_dir = save_dir + '/out_act/' + self.distiller_name + '/'
        # Path(save_dir ).mkdir(parents=True, exist_ok=True)
        #
        # dnn_act_data_df = pd.DataFrame(columns=['name', 'data'])
        # dnn_act_stats_df = pd.DataFrame(
        #     columns=['name', 'vol','shape', 'min', 'max', 'clipping','scale','zero_point'])
        # O_f = out_orig.view(-1)
        # O_f = O_f.to('cpu')
        # dnn_act_data_df = dnn_act_data_df.append(pd.Series({'name': self.distiller_name,
        #                 'data': O_f.detach().numpy()}), ignore_index=True)
        # shape_str = '{}'.format(out_f.shape).split('torch.Size')[-1]
        # dnn_act_stats_df = dnn_act_stats_df.append(pd.Series({'name': self.distiller_name,
        #                                                       'vol': O_f.shape[0],
        #                                                       'shape': shape_str,
        #                                                       'min': O_f.min().detach().numpy(),
        #                                                       'max': O_f.max().detach().numpy(),
        #                                                       'clipping': self.output_clipping[0].to('cpu').detach().numpy(),
        #                                                       'scale': self.output_scale[0].to('cpu').detach().numpy(),
        #                                                       'zero_point': self.output_zero_point[0].to('cpu').detach().numpy(),
        #                                                       }), ignore_index=True)
        # dnn_act_data_df.to_pickle(save_dir + '/act_data.pkl')
        # dnn_act_stats_df.to_pickle(save_dir + '/act_stats.pkl')
...
class RangeLinearQuantParamLayerWrapperBS(RangeLinearQuantWrapperBS):
    """
    ...
    Args:
        ...
        num_bits_accum (int): Number of bits allocated for the accumulator of intermediate integer results
        mode (ModuleQuantMode / LinearQuantMode): Quantization mode to use (symmetric / asymmetric-signed/unsigned)
        clip_acts (ClipNode): See RangeLinearQuantWrapperBS
        per_channel_wts (bool): Enable quantization of weights using separate quantization parameters per
            output channel
        activation_stats (dict): See RangeLinearQuantWrapperBS
        clip_n_stds (float): See RangeLinearQuantWrapperBS
        clip_half_range (bool) : See RangeLinearQuantWrapperBS
        scale_approx_mult_bits (int): See RangeLinearQuantWrapperBS
    """
    def __init__(self, wrapped_module, num_bits_acts, num_bits_params, num_bits_accum=32,
        ...
        super(RangeLinearQuantParamLayerWrapperBS, self).__init__(wrapped_module, num_bits_acts, num_bits_accum, mode,
    ...
    def named_linear_quant_params(self, filter=False):
        ...
        yield from super(RangeLinearQuantParamLayerWrapperBS, self).named_linear_quant_params(filter=filter)
    ...
    def state_dict(self, destination=None, prefix='', keep_vars=False):
        ...
        return super(RangeLinearQuantParamLayerWrapperBS, self).state_dict(destination, prefix, keep_vars)
    ...
    def extra_repr(self):
        ...
        tmpstr += super(RangeLinearQuantParamLayerWrapperBS, self).extra_repr()
...
class RangeLinearQuantMatmulWrapperBS(RangeLinearQuantWrapperBS):
    """
    ...
    Args:
        ...
        clip_acts (ClipNode): See RangeLinearQuantWrapperBS
        activation_stats (dict): See RangeLinearQuantWrapperBS
        clip_n_stds (int): See RangeLinearQuantWrapperBS
        scale_approx_mult_bits (int): See RangeLinearQuantWrapperBS
    """
    def __init__(self, wrapped_module, num_bits_acts, num_bits_accum=32,
        ...
        super(RangeLinearQuantMatmulWrapperBS, self).__init__(wrapped_module, num_bits_acts, num_bits_accum, mode,
                                                            clip_acts, activation_stats, clip_n_stds, clip_half_range,
                                                            scale_approx_mult_bits,
                                                            input_overrides=input_overrides,
                                                            inputs_quant_auto_fallback=inputs_quant_auto_fallback)
...
class RangeLinearQuantConcatWrapperBS(RangeLinearQuantWrapperBS):
    def __init__(self, wrapped_module, num_bits_acts, mode=LinearQuantMode.SYMMETRIC, clip_acts=ClipMode.NONE,
        ...
        super(RangeLinearQuantConcatWrapperBS, self).__init__(wrapped_module, num_bits_acts, mode=mode,
...
class RangeLinearQuantEltwiseAddWrapperBS(RangeLinearQuantWrapperBS):
    def __init__(self, wrapped_module, num_bits_acts, mode=LinearQuantMode.SYMMETRIC, clip_acts=ClipMode.NONE,
                 activation_stats=None, clip_n_stds=None, clip_half_range=False, scale_approx_mult_bits=None,
                 input_overrides=None, inputs_quant_auto_fallback=False):
        ...
        super(RangeLinearQuantEltwiseAddWrapperBS, self).__init__(wrapped_module, num_bits_acts, mode=mode,
                                                                clip_acts=clip_acts, activation_stats=activation_stats,
                                                                clip_n_stds=clip_n_stds, clip_half_range=clip_half_range,
                                                                scale_approx_mult_bits=scale_approx_mult_bits,
                                                                input_overrides=input_overrides,
...
class RangeLinearQuantEltwiseMultWrapperBS(RangeLinearQuantWrapperBS):
    def __init__(self, wrapped_module, num_bits_acts, mode=LinearQuantMode.SYMMETRIC, clip_acts=ClipMode.NONE,
                 activation_stats=None, clip_n_stds=None, clip_half_range=False, scale_approx_mult_bits=None,
                 input_overrides=None, inputs_quant_auto_fallback=False):
        ...
        super(RangeLinearQuantEltwiseMultWrapperBS, self).__init__(wrapped_module, num_bits_acts, mode=mode,
                                                                 clip_acts=clip_acts, activation_stats=activation_stats,
                                                                 clip_n_stds=clip_n_stds, clip_half_range=clip_half_range,
                                                                 scale_approx_mult_bits=scale_approx_mult_bits,
                                                                 input_overrides=input_overrides,
...
class RangeLinearEmbeddingWrapperBS(nn.Module):
    def __init__(self, wrapped_module, num_bits, mode=LinearQuantMode.SYMMETRIC, stats=None, save_fp_weights=False):
        ...
        super(RangeLinearEmbeddingWrapperBS, self).__init__()
...
class RangeLinearFakeQuantWrapper(RangeLinearQuantWrapperBS):
    def __init__(self, wrapped_module, num_bits_acts, mode=LinearQuantMode.SYMMETRIC, clip_acts=ClipMode.NONE,
                 activation_stats=None, clip_n_stds=None, clip_half_range=False, scale_approx_mult_bits=None,
...
_ptq_wrappers_int_only = (RangeLinearQuantWrapperBS, RangeLinearEmbeddingWrapperBS)
...
class PostTrainLinearQuantizerBitSearch(Quantizer):
    """
    ...
    def __init__(self, model, bits_activations=8, bits_parameters=8, bits_accum=32,
        ...
        super(PostTrainLinearQuantizerBitSearch, self).__init__(model, bits_activations=bits_activations,
        ...
        def replace_param_layer(module, name, qbits_map, per_channel_wts=per_channel_wts,
            ...
            return RangeLinearQuantParamLayerWrapperBS(module, qbits.acts, qbits.wts,
        ...
        def replace_embedding(module, name, qbits_map, fp16=fp16, fpq_module=fpq_module):
            ...
            norm_name = distiller.utils.normalize_module_name(name)
            return RangeLinearEmbeddingWrapperBS(module, qbits_map[name].wts, mode=mode,
        ...
        def replace_fake_quant(module, name, qbits_map, fp16=fp16,
            ...
            if isinstance(module, (nn.ReLU, nn.ReLU6)) and make_identity:
                ...
                if isinstance(named_modules[pred], RangeLinearQuantWrapperBS):
                    return nn.Identity()
        ...
        factory_concat = partial(
            replace_non_param_layer, RangeLinearQuantConcatWrapperBS)
        factory_eltwiseadd = partial(
            replace_non_param_layer, RangeLinearQuantEltwiseAddWrapperBS)
        factory_eltwisemult = partial(
            replace_non_param_layer, RangeLinearQuantEltwiseMultWrapperBS)
        factory_matmul = partial(
            replace_non_param_layer, RangeLinearQuantMatmulWrapperBS)
    ...
    def force_readjust_wrappers(self):
        def _force_readjust(module):
            if isinstance(module, RangeLinearQuantWrapperBS):
    ...
    def from_args(cls, model, args):
        ...
        if args.qe_config_file:
            return distiller.config_component_from_file_by_class(model, args.qe_config_file,
                                                                 'PostTrainLinearQuantizerBitSearch')
        else:
    ...
    def prepare_model(self, dummy_input=None):
        ...
        super(PostTrainLinearQuantizerBitSearch, self).prepare_model(dummy_input)
