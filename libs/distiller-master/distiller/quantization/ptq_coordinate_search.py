...
def init_linear_quant_params(quantizer, original_model, eval_fn, dummy_input, init_mode,
    ...
    layers_topological_order,_ = SummaryGraph(original_model, dummy_input).layers_topological_order()
    ...
    for module_name in layers_topological_order:
        ...
        tmp_xx = distiller.denormalize_module_name(quantizer.model, module_name)
        q_module = q_named_modules[tmp_xx]
...
def ptq_fake_quantization(quantizer, dummy_input, eval_fn, test_fn=None, method='Powell',
    ...
    if not quantizer.model_activation_stats:
        list_of_modules = None
        # list_of_modules = ['conv1',
        # 'maxpool']
        # 'layer1.0.conv1',
        ...
        act_stats = collect_quant_stats(model_temp, eval_fn,
                                        ...
                                        save_dir=getattr(msglogger, 'logdir', '.'), modules_to_collect=list_of_modules)
        del model_temp
...
def ptq_fake(quantizer, dummy_input, eval_fn, test_fn=None, method='Powell',
                          maxiter=None, maxfev=None, basinhopping=False, basinhopping_niter=100,
                          init_mode=ClipMode.NONE, init_method=None, search_clipping=False,
                          minimizer_kwargs=None):
    """
