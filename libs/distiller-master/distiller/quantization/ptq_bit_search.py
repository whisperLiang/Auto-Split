...
# from distiller.quantization.ptq_coordinate_search import *
...
def optimize_for_layer(layer, quantized_layer, loss_fn, input, method=None, search_clipping=False):
    ...
    * In this implementation:
    ...
    Args:
        ...
        quantized_layer (RangeLinearQuantWrapper or RangeLinearEmbeddingWrapperBS): the post-quantized layer.
...
def get_input_for_layer(model, layer_name, eval_fn):
    ...
    def hook_layer_input(module, input):
        # print('{} :: {} '.format(len(input), input[0].shape))
    ...
    if len(layer_inputs) == 1:
        val = layer_inputs[0]
        print(type(layer_inputs))
        # print(layer_inputs[0])
        print(layer_inputs[0].shape)
        print('len layers: {}'.format(len(layer_inputs)))
...
def init_layer_linear_quant_params(quantizer, original_model, layer_name, init_mode=ClipMode.NONE,
    ...
    if callable(init_mode):
        ...
        print('{}'.format(input_for_layer.shape))
...
def init_linear_quant_params(quantizer, original_model, eval_fn, dummy_input, init_mode,
    ...
    Args:
        ...
        searchF_clipping (bool): if set, optimize clipping values, otherwise optimize scale factor
    """
    original_model = distiller.make_non_parallel_copy(original_model)

    orig_model_df = model_summaries.model_performance_summary(original_model,dummy_input)
    save_dir=getattr(msglogger, 'logdir', '.')
    orig_model_df.to_csv(save_dir + '/orig_model_df.csv')


    layers_topological_order, rank_graph = SummaryGraph(original_model, dummy_input).layers_topological_order()
    save_dir=getattr(msglogger, 'logdir', '.')
    rank_graph.to_csv(save_dir + '/layer_ranks.csv')
    ...
    for module_name in layers_topological_order:
        ...
        init_layer_linear_quant_params(quantizer, original_model, module_name, module_init_mode,
                                       ...
                                       search_clipping=search_clipping)
    # del original_model
    # _,rank_graph2  = SummaryGraph(quantizer.model, dummy_input,optimize_graph=False).layers_topological_order()
    quantizer._post_prepare_model()
    # dump_weights_stats(quantizer.model)
    #TODO: collect activation numpy stats.
    compute_df = model_summaries.model_performance_summary(quantizer.model,dummy_input)
    compute_df = compute_df.drop_duplicates()
    save_dir=getattr(msglogger, 'logdir', '.')
    compute_df.to_csv(save_dir + '/post_prepare_model_df.csv')

    quantizer.model.eval()

def dump_weights_stats(model):
    # Load model
    dnn_wgt_data_df = pd.DataFrame(columns=['name','data'])
    dnn_wgt_stats_df = pd.DataFrame(
    ...
    # Load Model Weights
    layer_idx = 0
    for name, W0 in model.named_parameters():
        # eval(model.name.wrapped_module.float_weight)
        # W = W.to('cpu')
        if 'weight' in name:
            layer_name = name.split('.weight')[0]
            # var_no_grad = W.detach()
            # W.requires_grad=False
            # model.layer1[0].conv1.wrapped_module.float_weight

            # TODO: NOTE: This is a hack to extract float weights across recursive modules
            # A proper method will be to mimic .named_parameters() functions and get
            # .float_weight instead of .weight parameter
            names = layer_name.split('.')
            var_string=''
            for x in names:
                if not x.isnumeric():
                    var_string = '{}.{}'.format(var_string,x)
                else:
                    var_string = '{}[{}]'.format(var_string,x)
            # model.conv1.w_zero_point
            # model.conv1.w_scale
            # model.conv1.weight_clipping

            # model.conv1.output_clipping
            # model.conv1.fp_bias
            # model.conv1.output_scale
            # model.conv1.output_zero_point
            orig_layer_name = var_string.split('.wrapped_module')[0]
            W = eval('model' + var_string + '.float_weight')
            W = W.to('cpu')
            W = W.view(-1)

            w_zero_point = eval('model' + orig_layer_name + '.w_zero_point')
            w_scale = eval('model' + orig_layer_name + '.w_scale')
            weight_clipping = eval('model' + orig_layer_name + '.weight_clipping')

            dnn_wgt_data_df = dnn_wgt_data_df.append(pd.Series({'name': layer_name,
                                    'data':W.detach().numpy()}),
                                    ignore_index=True)
            dnn_wgt_stats_df  = dnn_wgt_stats_df.append(pd.Series({ 'name': layer_name,
                                    'vol':W.shape[0],
                                    'min': W.min().detach().numpy(),
                                    'max': W.max().detach().numpy(),
                                    'orig_layer_idx':layer_idx,
                                    'scale': w_scale.to('cpu').detach().numpy(),
                                    'zero_point': w_zero_point.to('cpu').detach().numpy(),
                                    'clipping': weight_clipping[0].to('cpu').detach().numpy()
                                    }), ignore_index=True)
            # dnn_wgt_data_df = dnn_wgt_data_df.append(pd.Series({'name': copy.deepcopy(layer_name),'data':copy.deepcopy(W.detach().numpy())}),
            #                        ignore_index=True)
            # dnn_wgt_stats_df  = dnn_wgt_stats_df.append(pd.Series({ 'name': copy.deepcopy(layer_name),'vol':copy.deepcopy(W.shape[0]),
            #                                             'min': copy.deepcopy(W.min().detach().numpy()),
            #                                             'max': copy.deepcopy(W.max().detach().numpy()),
            #                                             'orig_layer_idx':layer_idx
            #                                             }), ignore_index=True)
    ...
    save_dir = getattr(msglogger, 'logdir', '.')
    dnn_wgt_data_df.to_pickle(save_dir+'/weights_data.pkl')
    dnn_wgt_stats_df.to_pickle(save_dir+'/weights_stats.pkl')
    return dnn_wgt_data_df, dnn_wgt_stats_df
...
def validate_quantization_settings(quantized_model, search_clipping):
    ...
    for n, m in quantized_model.named_modules():
        ...
        if not isinstance(m, RangeLinearEmbeddingWrapperBS):
        ...
        if isinstance(m, (RangeLinearEmbeddingWrapperBS, RangeLinearQuantParamLayerWrapperBS)):
            if is_linear_quant_mode_asymmetric(m.mode.weights):
                raise ValueError(err_msg.format('weights'))


def ptq_bit_search(quantizer, dummy_input, eval_fn, test_fn=None, method='Powell',
                          ...
                          minimizer_kwargs=None, is_eval=True, is_fold_batch_norms=True):
    """
