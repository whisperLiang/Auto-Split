...
class QuantCalibrationStatsCollector(ActivationStatsCollector):
    ...
    def _check_required_stats(self):
        ...
        If not make sure not to quantize them and put them in overrides.
        """
        for name, module in self.model.named_modules():
            if not self._should_collect(module):
                continue
            if not hasattr(module, 'quant_stats'):
                msglogger.warning('Collection of Laplace distribution statistics is '
                                   'only allowed after collection of stats has started. : {}'.format(name))
                continue
    ...
    def start_second_pass(self):
        ...
        for module in self.model.modules():
            ...
            module.batch_idx = 0
            if not hasattr(module, 'quant_stats'):
                msglogger.warning('module: {} has no attribute quant_stats'.format(module.distiller_name))
                continue
    ...
    def _activation_stats_cb(self, module, inputs, output):
        ...
        # def empty_update_record(record):
        ...
        #
        #     if not record['shape']:
        #         record['shape'] = distiller.size2str(tensor.shape)
        ...
        with torch.no_grad():
            ...
            for idx, input in enumerate(inputs):
                # if module_name == 'Conv2d' or \
                #         module_name == 'Linear' or module_name == 'ReLU' or \
                #         module_name == 'BatchNorm2d' or module_name == 'MaxPool2d':
                if torch.is_tensor(input):
                    update_record(module.quant_stats.inputs[idx], input)
                else:
                    msglogger.warning('not a tensor: input: {} type: {} '.format(module.distiller_name, module.type ))
                    continue
                # if not torch.is_tensor(input):
                #     msglogger.warning('not a tensor: input: {} '.format(module.distiller_name))
                #     # empty_update_record(module.quant_stats.inputs[idx])
                #     continue
            ...
            if torch.is_tensor(input):
                ...
                msglogger.warning('not a tensor: output: {} type: {}'.format(module.distiller_name, module.type))
