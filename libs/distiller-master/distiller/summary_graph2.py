...
class SummaryGraph(object):
    ...
    def __init__(self, model, dummy_input, apply_scope_name_workarounds=True, optimize_graph=True):
        ...
        with torch.onnx.select_model_mode_for_export(model_clone, False):
            ...
            trace = torch.jit.trace(model.backbone, dummy_input, strict=False)
            trace2, _ = jit._get_trace_graph(model_clone.backbone, dummy_input, _force_outplace=True)
            ...
            # Not used the second time on getting trace for optimized graph
            if self._optimize_graph == True:
                torch.onnx._optimize_trace(trace2, torch.onnx.OperatorExportTypes.ONNX)
    ...
    def layers_topological_order(self, recurrent=False):
        ...
        rank_graph = pd.DataFrame(columns=['name','rank','pred','succ'])
        # Note: not the same as ret. ret Only contains ops from module_dict.
        sorted_rops = sorted([k for k in ranked_ops.keys()])
        for k in sorted_rops :
            op_rank = ranked_ops[k].rank
            pred_list = ranked_ops[k].adj_entry.predecessors
            pred_rank_list = [ranked_ops[k.name].rank for k in pred_list]
            pred_rank_list_str = ','.join([str(i) for i in pred_rank_list])

            succ_list = ranked_ops[k].adj_entry.successors
            succ_rank_list = [ranked_ops[k.name].rank for k in succ_list]
            succ_rank_list_str = ','.join([str(i) for i in succ_rank_list])

            rank_graph = rank_graph.append([{'name':k,'rank':op_rank,
            'pred': pred_rank_list_str,'succ':succ_rank_list_str}], ignore_index=True)
