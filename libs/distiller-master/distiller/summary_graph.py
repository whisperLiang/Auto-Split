...
class SummaryGraph(object):
    ...
    def __init__(self, model, dummy_input, apply_scope_name_workarounds=True, optimize_graph=True):
        ...
        with torch.onnx.set_training(model_clone, False):
            ...
            # trace2, _ = jit._get_trace_graph(model_clone, dummy_input) # works in pytorch 1.4
            # and then use for node in trace2.nodes() pytorch 1.4
            # for trace2.graph.nodes() in pytorch 1.3
    ...
    def layers_topological_order(self, recurrent=False):
        ...
        ret2 = sorted([k for k in ranked_ops.keys()],
                     key=lambda k: ranked_ops[k].rank)
        # assign my_rank
        my_rank = list(range(len(ret2)))
        for idx, key in zip(my_rank, ret2):
            # op_rank = ranked_ops[key].rank
            op_rank = idx
            #ranked_ops[k.name].rank not used.
            # Since, rank is not unique which comes from distiller
            ...
            rank_graph = rank_graph.append([{'name': key, 'rank': op_rank,
                                             'pred': pred_rank_list_str, 'succ': succ_rank_list_str}],
                                           ignore_index=True)
