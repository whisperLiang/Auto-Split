...
from detection_models.yolov3_master.utils.utils import  xyxy2xywh, xywh2xyxy
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, cocodir_path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, pad=0.0):


        subset_size=None
        # subset_size = 10
        data_dir = os.path.abspath(os.path.join(path, os.pardir))
        # coco_dir = os.path.abspath(os.path.join(data_dir, os.pardir))
        ...
        try:
            ...
            # f = self.add_coco_path_prefix(f, coco_img_path='/data/datasets/coco2017/val2017')
            # self.img_files = [x.replace('/', os.sep) for x in f if os.path.splitext(x)[-1].lower() in img_formats]
            with open(path, 'r') as f:
                self.img_files = [x.replace('/', os.sep) for x in f.read().splitlines()  # os-agnostic
                                  if os.path.splitext(x)[-1].lower() in img_formats]
                self.img_files = [x.replace('..', coco_dir) for x in self.img_files]
        ...
        if subset_size is not None:
            self.img_files = self.img_files[0:subset_size]
        ...
        self.label_files = [
            x.replace(coco_dir, os.path.join(data_dir, 'labels')).replace(os.path.splitext(x)[-1], '.txt') for x in
            self.img_files]
    ...
    # def add_coco_path_prefix(self, f, coco_img_path= os.path.join(self.coco_dir, '/val2017')):
    #     f = [coco_img_path + '/' + x.split('/')[-1] for x in f]
    #     return f
    def __len__(self):
