...
from tools.bit_search.detection_compression.engine import evaluate

msglogger = logging.getLogger()

def detection_ptq_lapq(model, eval_data_loader , test_data_loader, args):
    args = deepcopy(args)
    model = model.eval()
    device = next(model.parameters()).device

    def eval_fn(model):
        _, _, loss = evaluate(model, eval_data_loader, device=device)
        return loss


    def test_fn(model):
        return evaluate(model, test_data_loader, device=device)
    ...
    # model.input_shape = tuple(3,480,640)
...
if __name__ == "__main__":
    ...
    detection_ptq_lapq(cc.model, cc.criterion, [cc.pylogger, cc.tflogger], cc.args)

    # if args.evaluate:
    #     if args.quantize_eval and args.qe_lapq:
    #         detection_ptq_lapq(model, criterion, pylogger, args)