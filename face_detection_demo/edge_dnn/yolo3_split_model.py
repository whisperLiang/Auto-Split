...
def YoloSplit(self, config, training=False, norm_decay=0.997, norm_epsilon=1e-5):
    edge_dnn = EdgeDNN(training, config)
    cloud_dnn = CloudDNN(training, config)
    conv, conv_index = edge_dnn._darknet53_edge(None, training, norm_decay, norm_epsilon)
    self.boxes, self.scores, self.classes = cloud_dnn._darknet53_cloud(conv, conv_index, training, norm_decay, norm_epsilon)
